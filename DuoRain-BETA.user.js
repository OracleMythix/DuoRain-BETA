...
(function() {
    ...
    function injectUI() {
                       ...
                       <div class="DLP_HStack_Auto_Top DLP_NoSelect">
                        ...
                        <div id="duorain-status-area">
                            ...
                            <div id="duorain-running-farms-list">
                                <div><!-- Inner container for transition --></div>
                            </div>
                        </div>
        ...
        const uiStyle = `
            ...
            .DLP_Text_Style_1 { font-family: "DuoRain", sans-serif; font-size: 16px; font-weight: 500; margin: 0; color: #FFF;}
            .DLP_Text_Style_2 { font-family: "DuoRain", sans-serif; font-size: 24px; font-weight: 500; margin: 0; color: #FFF;}
            ...
            .DLP_Main_Box { display: flex; width: 340px; padding: 24px 20px; box-sizing: border-box; flex-direction: column; gap: 8px; border-radius: 24px; background: rgba(45, 55, 75, 0.6); backdrop-filter: blur(16px) saturate(180%); -webkit-backdrop-filter: blur(16px) saturate(180%); box-shadow: 0 10px 40px rgba(0,0,0,0.25); border: 1px solid rgba(255, 255, 255, 0.1); color: #FFF; }
            .DLP_HStack_Auto_Top { display: flex; align-items: flex-start; justify-content: space-between; align-self: stretch; }
            .DLP_HStack_4 { display: flex; align-items: center; gap: 4px; align-self: stretch; }
            .DLP_HStack_8 { display: flex; align-items: center; gap: 8px; align-self: stretch; }
            ...
            .DLP_Input_Style_1_Active { display: flex; height: 48px; padding: 16px; box-sizing: border-box; align-items: center; flex: 1 0 0; gap: 6px; border-radius: 8px; outline: 2px solid rgba(0, 122, 255, 0.20); outline-offset: -2px; background: rgba(0, 0, 0, 0.2); }
            ...
            .DLP_Input_Input_Style_1 { border: none; outline: none; background: none; text-align: left; font-family: "DuoRain", sans-serif; font-size: 16px; font-weight: 500; color: #89CFF0; width: 100%; }
            .DLP_Input_Input_Style_1::placeholder { color: rgba(137, 207, 240, 0.5); }
            ...
            .DLP_Input_Input_Style_1[type=number] { -moz-appearance: textfield; }

            /* Status Box */
            #duorain-status-area { margin-bottom: 8px; }
            #duorain-status-indicator { justify-content: center; transition: all 0.3s ease; }
            #duorain-status-indicator.idle { background-color: rgba(120, 120, 128, 0.2); }
            #duorain-status-indicator.idle p { color: rgba(255,255,255,0.6); }
            #duorain-status-indicator.running { background-color: rgba(255, 149, 0, 0.3); }
            #duorain-status-indicator.running p { color: #FFD580; }
            ...
            .duorain-farm-status-box { display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; border-radius: 12px; background-color: rgba(0,0,0,0.2); }
            .duorain-farm-status-box .status-text { color: white; font-size: 14px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;}
            ...
            .duorain-farm-status-box .duorain-button-stop:hover { filter: brightness(1.1); }
            .duorain-farm-status-box .duorain-button-stop:disabled { background-color: #7d7d7d; cursor: not-allowed; }

            #duorain-toggle-button { position: fixed; bottom: 20px; right: 20px; background-image: linear-gradient(45deg, #007AFF, #5AC8FA); color: white; padding: 12px 18px; border-radius: 50px; cursor: pointer; font-family: "DuoRain", sans-serif; font-weight: bold; box-shadow: 0 5px 20px rgba(0, 122, 255, 0.35); z-index: 10000; transition: transform 0.2s, box-shadow 0.2s; }
        `;
    ...
    function stopFarm(farmId, isManual = true) {
        ...
        activeFarms.set(farmId, false); // Signal loop to stop
    ...
    async function farmXp(jwt, fromLang, toLang, count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const payload = {
                "awardXp": true, "completedBonusChallenge": true, "fromLanguage": fromLang, "learningLanguage": toLang, "hasXpBoost": false,
                "illustrationFormat": "svg", "isFeaturedStoryInPracticeHub": true, "isLegendaryMode": true, "isV2Redo": false,
                "isV2Story": false, "masterVersion": true, "maxScore": 0, "score": 0, "happyHourBonusXp": 469,
    ...
    if (document.readyState === 'loading') { window.addEventListener('DOMContentLoaded', main); } else { main(); }
